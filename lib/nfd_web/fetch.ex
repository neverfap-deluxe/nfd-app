defmodule NfdWeb.Fetch do
  ...
  def fetch_content(conn, page_view, page_symbol, slug, page_layout, collection_array) do
    ...
    case apply(ContentAPI, page_symbol, [client, verified_slug]) do
      {:ok, response} ->
        ...
        content_collections = FetchCollection.content_collections(client, response.body["data"], page_symbol, verified_slug, user_collections, collection_array)
        ...
        conn
          ...
          |> put_view(page_view)
          |> render("#{Atom.to_string(page_symbol)}.html", layout: { NfdWeb.LayoutView, page_layout }, item: response.body["data"], user_collections: user_collections, content_collections: content_collections, changeset_collections: changeset_collections, content_collections: content_collections, page_type: "content")
  ...
  def fetch_dashboard_collection(conn, page_symbol, collection_slug, file_slug, collection_array) do
    ...
    conn
      |> FetchConn.is_collection_complete(page_symbol, user_collections, dashboard_collections, NfdWeb.DashboardView, "hub.html", "dashboard_no_complete.html")
      |> put_flash(:info, (if user_collections.patreon_access.token_expired, do: "Welcome back!", else: "Your Patreon token has expired. Please Re-link your account."))
      |> put_view(NfdWeb.DashboardView)
  ...
  def fetch_dashboard_file(conn, page_symbol, collection_slug, file_slug, collection_array) do
    ...
    conn
      |> FetchConn.is_collection_complete(page_symbol, user_collections, dashboard_collections, NfdWeb.DashboardView, "hub.html", "dashboard_no_complete.html")
      |> FetchConn.is_file_paid_for(page_symbol, user_collections, dashboard_collections, NfdWeb.DashboardView, "hub.html", "dashboard_no_access.html")
      |> FetchConn.are_they_up_to_day(page_symbol, Map.get(dashboard_collections, :file_content), user_collections, dashboard_collections, NfdWeb.DashboardView, "hub.html", "dashboard_no_access_up_top.html")
      |> put_flash(:info, (if user_collections.patreon_access.token_expired, do: "Welcome back!", else: "Your Patreon token has expired. Please Re-link your account."))
      |> put_view(NfdWeb.DashboardView)
