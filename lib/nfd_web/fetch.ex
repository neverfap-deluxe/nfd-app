defmodule NfdWeb.Fetch do
  ...
  def fetch_content(conn, page_view, page_symbol, slug, page_layout, collection_array) do
    ...
    case apply(ContentAPI, page_symbol, [client, verified_slug]) do
      {:ok, response} ->
        ...
        conn
          |> is_file_paid_for(page_symbol, user_collections, item_collections, NfdWeb.DashboardView, "dashboard_no_access.html")
          |> are_they_up_to_day(page_symbol, response, user_collections, item_collections, NfdWeb.PageView, "page_not_up_to.html")
  ...
  def fetch_dashboard(conn, page_symbol, collection_slug, file_slug, collection_array) do
    ...
    conn
      ...
      |> is_file_paid_for(page_symbol, user_collections, dashboard_collections, NfdWeb.DashboardView, "dashboard_no_complete.html")
      |> are_they_up_to_day(page_symbol, dashboard_collections.file_page_information, user_collections, dashboard_collections, NfdWeb.DashboardView, "dashboard_no_access_up_top.html")
  ...
  def are_they_up_to_day(conn, page_symbol, user_collections, dashboard_collections, view, template) do
    day = response.body["data"]["day"]
    case page_symbol do
      :seven_day_kickstarter_single -> if user_collections.subscriber |> Map.get(:seven_day_kickstarter_count) <= day, do: conn, else: render_no_access_page(conn, dashboard_collections, view, template)
      :ten_day_meditation_single -> if user_collections.subscriber |> Map.get(:ten_day_meditation_count) <= day, do: conn, else: render_no_access_page(conn, dashboard_collections, view, template)
      :twenty_eight_day_awareness_single -> if user_collections.subscriber |> Map.get(:twenty_eight_day_awareness_count) <= day, do: conn, else: render_no_access_page(conn, dashboard_collections, view, template)
      :seven_week_awareness_vol_1_single -> if user_collections.subscriber |> Map.get(:awareness_seven_week_vol_1_count) <= day, do: conn, else: render_no_access_page(conn, dashboard_collections, view, template)
      :seven_week_awareness_vol_2_single -> if user_collections.subscriber |> Map.get(:awareness_seven_week_vol_2_count) <= day, do: conn, else: render_no_access_page(conn, dashboard_collections, view, template)
      :seven_week_awareness_vol_3_single -> if user_collections.subscriber |> Map.get(:awareness_seven_week_vol_3_count) <= day, do: conn, else: render_no_access_page(conn, dashboard_collections, view, template)
      :seven_week_awareness_vol_4_single -> if user_collections.subscriber |> Map.get(:awareness_seven_week_vol_4_count) <= day, do: conn, else: render_no_access_page(conn, dashboard_collections, view, template)
      _ -> conn
    end
  end
  ...
  def is_collection_complete(conn, page_symbol, user_collections, dashboard_collections) do
    case page_symbol do
        page_symbol when page_symbol in [:dashboard_course_collection, :dashboard_course_file] ->
          if dashboard_collections.course.status == "complete", do: conn, else: render_no_access_page(conn, dashboard_collections, NfdWeb.DashboardView, "dashboard_no_complete.html")

        page_symbol when page_symbol in [:dashboard_ebook_collection, :dashboard_ebook_file] ->
          if dashboard_collections.ebook.status == "complete", do: conn, else: render_no_access_page(conn, dashboard_collections, NfdWeb.DashboardView, "dashboard_no_complete.html")
  ...
  def is_file_paid_for(conn, page_symbol, user_collections, dashboard_collections, view, template) do
    case page_symbol do
      :dashboard_course_file ->
        ...
        if !!has_patreon_access or !!has_paid_for_collection, do: conn, else: render_no_access_page(conn, dashboard_collections, view, template)
  ...
  def render_no_access_page(conn, dashboard_collections, view, template) do
    conn
      |> put_view(view)
