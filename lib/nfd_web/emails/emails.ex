defmodule NfdWeb.Emails do
  ...
  def send_day_0_email(subscriber, main_matrix) do
    ...
    type = Util.Email.main_matrix_to_type(main_matrix)

    IO.inspect type

    cond do
      # TODO: Welcome email for general newsletter.
      type == general_type -> nil
      type == kickstarter_type -> email_scheduler_logic(kickstarter_type, subscriber, subscriber.seven_day_kickstarter_subscribed, 0)
      type == meditation_primer_type -> email_scheduler_logic(meditation_primer_type, subscriber, subscriber.ten_day_meditation_subscribed, 0)
      type == awareness_challenge_type -> email_scheduler_logic(awareness_challenge_type, subscriber, subscriber.twenty_eight_day_awareness_subscribed, 0)
    end
  end
  ...
  def email_scheduler_logic(type, subscriber, is_subscribed, day_count) do
    ...
    if (is_subscribed) do
      { general_type, kickstarter_type, meditation_primer_type, awareness_challenge_type} = Util.Email.generate_course_types()
      unsubscribe_url = Util.Email.course_type_to_matrix(type) |> Util.Email.generate_unsubscribe_url(subscriber.subscriber_email)
      
      { template, subject } =
        cond do
          type == kickstarter_type -> EmailTemplates.run_seven_day_kickstarter(day_count)
          type == meditation_primer_type -> EmailTemplates.run_ten_day_primer(day_count)
          type == awareness_challenge_type -> EmailTemplates.run_twenty_eight_day_challenge(day_count)
        end

      cond do 
        type == kickstarter_type -> cast_course_email(subscriber, subject, template, unsubscribe_url) |> process("Seven Day NeverFap Deluxe Kickstarter - Day #{day_count} E-mail sent: " <> subscriber.subscriber_email)
        type == meditation_primer_type -> cast_course_email(subscriber, subject, template, unsubscribe_url) |> process("Ten Day Meditation Primer - Day #{day_count} E-mail sent: " <> subscriber.subscriber_email)
        type == awareness_challenge_type -> cast_course_email(subscriber, subject, template, unsubscribe_url) |> process("Twenty Eight Day Challenge - Day #{day_count} E-mail sent: " <> subscriber.subscriber_email)
      end
      
      cond do
        type == kickstarter_type -> update_subscription(type, subscriber, 7, day_count)
        type == meditation_primer_type -> update_subscription(type, subscriber, 10, day_count)
        type == awareness_challenge_type -> update_subscription(type, subscriber, 28, day_count)
      end
    end
  end

  def update_subscription(type, subscriber, subscription_day_limit, day_count) do
    { count_property, subscribed_property } = Util.Email.type_to_subscriber_properties(type)
    ...
    case Meta.create_subscription_email(%{ day: day_count, course: type, subscription_email: subscriber.subscriber_email}) do
      {:ok, subscription_email} ->
        case day_count == subscription_day_limit do 
          true ->
            Account.update_subscriber(subscriber, %{ count_property => 0, subscribed_property => false })
          false ->
            Account.update_subscriber(subscriber, %{ count_property => Map.fetch!(subscriber, count_property) + 1 })
        end
